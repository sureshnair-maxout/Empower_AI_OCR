...
Simplified version that uses PaddleOCR's documented API.
"""
...
def run_paddle_ocr(file_path: Path, lang: str = 'en') -> List[Dict]:
    ...
    # predict() returns a list of OCRResult objects
    results = ocr.predict(str(file_path))
    
    ocr_lines = []
    
    for page_result in results:
        # Access the detected text via result's raw output
        # Each result has a string representation that we need to parse
        #Convert to JSON format
        result_dict = json.loads(str(page_result))
        
        for item in result_dict.get('dt_polys', []):
